...
def check_offer(url: str):
    ...
    if resp.status_code == 403:
        return {"status": "ZABLOKOWANE PRZEZ STRONĘ", "price": None, "currency": None,
                "location": None, "title": None, "mileage": None, "image": None,
                "note": ("HTTP 403 - strona zablokowała zapytanie (często dotyczy serwerów "
                         "GitHub Actions, bo mają współdzielone adresy IP, które bywają na "
                         "czarnych listach antybotowych). To nie znaczy, że oferta zniknęła.")}

    if resp.status_code >= 400:
        return {"status": "BŁĄD SERWERA", "price": None, "currency": None,
                "location": None, "title": None, "mileage": None, "image": None,
